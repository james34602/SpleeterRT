...
void col2im_add_pixel_dilated(float *im, int height, int width, int row, int col, int channel, int pad, float val)
{
	row -= pad;
	col -= pad;
	...
	for (c = 0; c < channels_col; ++c)
	{
		...
		for (h = 0; h < height_col; ++h)
		{
			for (w = 0; w < width_col; ++w)
			{
				...
				col2im_add_pixel_dilated(data_im, height, width, im_row, im_col, c_im, pad, val);
			}
		}
	}
}
...
{
	return (dim - 1) * stride - 2 * pad + kernelSize + outputPadding;
}